...
public class RightRed extends LinearOpMode
    ...
    public void runOpMode() throws InterruptedException{
        ...
        //Thread.sleep(1000);
        int[] values = {valLeft,valMid,valRight};
    ...
    public void encoderDrive(double inches, double pow, String driveMode) throws InterruptedException {
        if(driveMode.equals("drive")){
            ...
            pidDrive.enable();
            resetAngle();
            int startPos1 = robot.frontLeftWheel.getCurrentPosition();
            int startPos2 = robot.backLeftWheel.getCurrentPosition();
            int startPos3 = robot.frontRightWheel.getCurrentPosition();
            int startPos4 = robot.backRightWheel.getCurrentPosition();
            robot.frontLeftWheel.setTargetPosition((int)(inches*COUNTS_PER_INCH));
            robot.backRightWheel.setTargetPosition((int)(inches*COUNTS_PER_INCH));
            robot.frontRightWheel.setTargetPosition((int)(inches*COUNTS_PER_INCH));
            robot.backLeftWheel.setTargetPosition((int)(inches*COUNTS_PER_INCH));
            double currentPosInches;
            if(inches>=0) {
                //robot.changeSpeed(power);
                while (robot.frontRightWheel.getTargetPosition()>robot.frontRightWheel.getCurrentPosition()||
                        robot.frontLeftWheel.getTargetPosition()>robot.frontLeftWheel.getCurrentPosition()||
                        robot.backRightWheel.getTargetPosition()>robot.backRightWheel.getCurrentPosition()||
                        robot.backLeftWheel.getTargetPosition()>robot.backLeftWheel.getCurrentPosition()) {
                    correction = pidDrive.performPID(getAngle());
                    currentPosInches = ((robot.frontLeftWheel.getCurrentPosition() - startPos1) / COUNTS_PER_INCH);
                    teleUpdate("CURRENTPOSINCHES: "+currentPosInches+"","");
                    power = function.getPowerAt(currentPosInches, inches, pow, "drive");
                    robot.frontLeftWheel.setPower(power - correction);
                    robot.backRightWheel.setPower(power + correction);
                    robot.frontRightWheel.setPower(power + correction);
                    robot.backLeftWheel.setPower(power - correction);
                }
            }
            else{
                //robot.changeSpeed(-power);
                while (robot.frontRightWheel.getTargetPosition()<robot.frontRightWheel.getCurrentPosition()||
                        ...
                        robot.backLeftWheel.getTargetPosition()<robot.backLeftWheel.getCurrentPosition()) {
                    correction = pidDrive.performPID(getAngle());
                    currentPosInches = ((robot.frontLeftWheel.getCurrentPosition() - startPos1) / COUNTS_PER_INCH * -1);
                    power = -function.getPowerAt(currentPosInches, -inches, pow, "drive");
            ...
            if(inches>0) {
                while (robot.frontRightWheel.getTargetPosition()>robot.frontRightWheel.getCurrentPosition()||
                    ...
                    telemetry.update();
                    correction = pidDrive.performPID(getAngle());
                    currentPosInches = ((robot.frontRightWheel.getCurrentPosition() - startPos1) / COUNTS_PER_INCH);
                    power = function.getPowerAt(currentPosInches, inches, pow, "strafe");
                    robot.frontLeftWheel.setPower(-(power + correction));
                    robot.backRightWheel.setPower(-(power - correction));
                    robot.frontRightWheel.setPower((power + correction));
                    robot.backLeftWheel.setPower((power - correction));             //STRAFE
                }
            }
            else{
                while (robot.frontRightWheel.getTargetPosition()<robot.frontRightWheel.getCurrentPosition()||
                    ...
                    telemetry.update();
                    correction = pidDrive.performPID(getAngle());
                    currentPosInches = ((robot.frontRightWheel.getCurrentPosition() - startPos1) / COUNTS_PER_INCH * -1);
                    power = -function.getPowerAt(currentPosInches, -inches, pow, "strafe");
                    robot.frontLeftWheel.setPower(-(power + correction));
                    robot.backRightWheel.setPower(-(power - correction));
                    robot.frontRightWheel.setPower((power + correction));
    ...
    public void teleUpdate(String label, String description){
        if (robot != null && robot.frontLeftWheel != null && robot.frontRightWheel != null) {
            telemetry.addLine().addData("Current Position",  "Running at %7d :%7d", robot.frontLeftWheel.getCurrentPosition(), robot.frontRightWheel.getCurrentPosition());
        }
