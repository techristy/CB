...
//get ready to shoot
    ...
    public void semiTurn(String type, int angle) throws InterruptedException{
        resetAngle();
        ...
        if(type.equals("clockwise")){
            long time = System.currentTimeMillis();
            while (getAngle() >= -angle && (System.currentTimeMillis()<(time+3000))) {
                power = (.75*2*0.684/5.063) * (-Math.pow((((-getAngle())+2.9)/37.4),2) + 4.5*((-getAngle()+2.9)/37.4)) + 0.159;
